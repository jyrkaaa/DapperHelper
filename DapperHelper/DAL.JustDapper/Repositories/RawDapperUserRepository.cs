...
namespace DAL.JustDapper.Repositories;

public class RawDapperUserRepository(IDbConnectionFactory connectionFactory) : IUserRepository
{
    public async Task<IEnumerable<UserEntity>> GetAllAsync()
    {
        using var conn = await connectionFactory.CreateConnectionAsync();
        return await conn.QueryAsync<UserEntity>(@"select * from Users");
    }

    public async Task<UserEntity?> GetByIdAsync(int id)
    {
        using var conn = await connectionFactory.CreateConnectionAsync();
        return await conn.QuerySingleOrDefaultAsync<UserEntity>(@"select * from Users where Id = @Id",
            new { Id = id });
    }
    ...
    {
        using var conn = await connectionFactory.CreateConnectionAsync();
        return await conn.QuerySingleOrDefaultAsync<UserEntity>("select * from Users where Email = @Email and Password = @Password",
